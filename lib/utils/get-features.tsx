...
export function getAllFeatures(features: Features, amenities: Amenities): FeatureItem[] {
      ...
      {filteredFeatures.map(feature => (
        ...
        >
          {feature.status === 'yes' ? (
            <Check
              size={16}
              style={{ color: 'var(--pico-card-background-color)', flexShrink: 0 }}
            />
          ) : (
            <X size={16} style={{ color: 'var(--pico-del-color)', flexShrink: 0 }} />
          )}
          <span
            style={{
              fontSize: '0.9rem',
...
export function FeatureTags({
  ...
  const allFeatures = amenities ? getAllFeatures(features, amenities) : getFeatures(features)
